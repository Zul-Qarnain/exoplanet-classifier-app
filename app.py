...
import pandas as pd
import json
import torch  # Required by the TabNet model
import plotly.graph_objects as go

@st.cache_resource
def load_model_and_features():
    """Load the model and feature names from the project directory."""
    try:
        # Paths are now local to the repository
        model = joblib.load('tabnet_exoplanet.pkl')
        with open('feature_list.json', 'r') as f:
            features = json.load(f)
        return model, features
    except FileNotFoundError:
        st.error("Error: `tabnet_exoplanet.pkl` or `feature_list.json` not found.")
        return None, None
